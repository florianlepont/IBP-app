...
| The 2026-09 code audit's data-loss, sync-integrity and account-safety lots run in this milestone as Phases 1.2–1.5, before the field tests; architecture, performance, i18n and hygiene lots move to the next milestone | Several findings (session error wipes offline data, concurrent drains, unbounded batches, submit bypass through `/sync`) directly contradict the core value; field tests on top of them would lose data | — Pending |
