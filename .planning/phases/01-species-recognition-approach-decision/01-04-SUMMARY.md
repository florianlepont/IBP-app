...
provides:
  ...
  - "ITERATION 4 (further deviation, reconnaissance-only checkpoint, 2026-09-25): user asked for a corpus/model rebalance targeted at the Ile-de-France temperate genera the model is weakest on (Fraxinus, Ulmus, Tilia, Carpinus, Acer, Alnus, Populus, Prunus, Betula, Fagus, Quercus deciduous), specifically citing spring as the worst season. Per the task's explicit 'report back before committing to a long download' instruction, this pass is source reconnaissance only -- no corpus expansion, retrain or export has happened. Added as measurement document Section 13. Headline finding CORRECTS Section 11.5's 'real property of GBIF's holdings, not a sampling artefact' conclusion for Acer/Pinus/Prunus autumn scarcity: re-querying GBIF directly with a month filter (never used by prepare_dataset.py's fetch) returns 40,603 / 23,517 / 34,016 autumn-dated, licence-clean StillImage records for Acer/Prunus/Pinus respectively, four to five orders of magnitude above what the corpus actually captured (66/30/60) -- root-caused to fetch_candidates_for_key's non-season-aware sequential fetch plus an early-exit heuristic (prepare_dataset.py:346) that is unsound for high-supply genera. Same pattern holds generally, not just for those three: every weak temperate genus checked has 2.3x-83.5x headroom against GBIF's true CC0+CC-BY StillImage pool versus what the 6,000/class target actually captured, while the Mediterranean/evergreen genera the user is already happy with are close to their true, much smaller ceilings -- the flat per-class cap was self-imposed, not GBIF-imposed. Four alternative sources investigated: Pl@ntNet-300K rejected outright (CC-BY-4.0 confirmed, but its 1,081-species list contains zero of the 34 CNPF genera -- checked directly against the metadata file, not assumed); Tela Botanica checked and blocked on SSO-gated API access (its CC-BY-SA 2.0 FR licence would clear the gate, and its GBIF-published CEL dataset is confirmed to carry zero StillImage-tagged records today, i.e. a genuinely untapped pool -- but every api-cel.tela-botanica.org endpoint, including its own API docs, 403s without a logged-in session, flagged as needing an explicit human decision rather than a scripted workaround); iNaturalist-direct usable but largely redundant with what GBIF's own iNaturalist-sourced records already offer; Wikimedia Commons usable and licence-verifiable per-file, moderate scale, best suited as a supplement for residual seasonal gaps (found genus-level 'in autumn' categories directly). Recommendation: fix and rescale the existing GBIF pipeline first (no new licence risk, order-of-magnitude headroom already confirmed) and only reach for Wikimedia Commons or a resolved Tela Botanica access path if a gap remains after that -- before spending the hours new-source integration would cost. Levers 3 (longer training schedule) and 4 (stronger backbone) deferred pending this corpus decision."
  - "ITERATION 4 EXECUTION (2026-09-25), coordinator-approved after independent verification of the Section 13.1 finding: proceed with the pipeline-fix-first plan, do not pursue Tela Botanica for now (parked, not closed). Corpus collection complete -- added as measurement document Section 14.1. Fixed prepare_dataset.py: widened (not removed) the early-exit heuristic that was unsound for high-supply genera (2 pages' worth of zero-progress offset -> 8), added season-scoped fetching via GBIF's month= filter, raised the per-class target to 10,000 (from 6,000) for exactly the 20 weak temperate genera with confirmed headroom, and routed Pinus through the same season-scoped fetch at its UNCHANGED 6,000 target (named separately by the coordinator for its autumn gap specifically, not on the cap-raise list). Result: corpus grew from 194,653 (iteration 3) to 265,546 selected images (269,341 raw files retained on disk), a deliberately moderate 1.36x increase next to iteration 3's 3.05x jump. Autumn counts for the three previously-unmeasurable genera: Acer 66->2,500 (37.9x), Prunus 30->2,500 (83.3x), Pinus 60->1,500 (25.0x) -- both directly reopening Section 12.6's 'cannot tell, not a resolvable gap' verdict. verify_corpus_complete() PASSED before training started. A first (unscoped) run wasted time topping up non-target, already-near-true-ceiling classes (e.g. Ceratonia at ~0.1-0.2 img/s) before being killed cleanly (no corruption, per-class checkpointing already in place) and restarted with a new --only-classes/--only-weak-temperate CLI restriction. Training launched (finetune.py --backbone efficientnet_b0 --output-suffix _v4 --head-epochs 8 --finetune-epochs 20 --unfreeze-last-n-layers 76, PID 19098, 2026-09-25 13:29 CEST) -- EfficientNetB0 chosen over B3 given this fanless MacBook Air's own iteration-3 memory pressure; EarlyStopping(monitor=val_top3, mode=max, patience=3, restore_best_weights=True) confirmed present and unchanged, verified directly against the running process's source file. A genuine memory bug was found and fixed before launch: make_dataset()'s per-epoch shuffle(8192) ran AFTER decode (on ~602KB float32 image tensors, ~4.9GB buffer regardless of corpus size) rather than before it (on lightweight path/label pairs) -- moved to run pre-decode, matching standard tf.data ordering. Training process RSS 1.04GB (not itself memory-heavy); machine-wide load average (13.69/22.91/25.23) and swap (2.79/4GB) observed during the run are consistent with pre-existing shared-machine contention present before this iteration started (15/16GB used, 6GB compressed, recorded before any iteration-4 work began), not a new leak in this pipeline. Expected training duration: ~2.2h head phase (observed directly) plus an estimated 7-10h fine-tune phase if EarlyStopping does not fire early, for a total iteration-4 wall-clock plausibly in the 5-12h range -- actual duration and the epoch EarlyStopping stops at recorded in Section 14.3 once complete."
  - "ITERATION 4 COMPLETE (2026-09-26): training finished at 84,477.7s (23.47h) total wall-clock -- 5.80h head phase (8 epochs) + 17.66h finetune phase (20 of 20 epochs run, EarlyStopping did NOT fire). best_finetune_epoch_1indexed=20 -- val_top3 posted a marginal new high every single epoch (final 0.8837), but val_loss bottomed at epoch 15 (0.9549) and rose in 4 of the next 5 epochs while train loss kept falling sharply -- genuine overfitting onset that val_top3-based EarlyStopping (patience=3) is too coarse to catch. This flips iteration 3's schedule-limited finding: this backbone/corpus combination has reached saturation, no longer schedule-limited. Exported genus_classifier_v4.tflite: 8,238,676 bytes (8.24MB, up from v3's 6.13MB as expected for EfficientNetB0's larger param count) -- plan 01-05 must re-time this on device. Parity: 100% top-1 agreement (68/68), float16 quantisation, cleaner than v3's one flip. Evaluated on the test split (26,557 images, up from 19,466): pooled top1 67.42%->72.08%, top3 85.24%->88.25% (+3.02pp); 32/34 genera improved (unweighted mean +3.67pp); still 1/34 clears D-02 (Tamarix, 96.88%, up from 95.04%; no new partial-go). THE decisive split: Ile-de-France temperate genera (21, targeted) improved by mean +4.93pp top-3, 21/21 positive; Mediterranean/evergreen/other (13, untouched) improved by mean +1.65pp, 11/13 positive (Olea -0.51pp, Pistacia -0.18pp, both noise-level) -- the targeted rebalance worked ~3x better on the genera it targeted than the general capacity/schedule improvement did on everything else. Per-genus x per-season (Section 12 format): all 136 cells now clear n>=30 (previously 5 thin, including Acer/Pinus/Prunus autumn). Acer/Pinus/Prunus autumn now measurable for the first time: Acer 82.40% (n=250, holds up vs 80.90% whole-year), Prunus 81.60% (n=250, a real -2.8pp autumn-specific weakness), Pinus 95.35% (n=172, clears D-02 in isolation, its strongest season) -- directly resolving Section 12.6's 'cannot tell' verdict. Winter remains the weakest season (85.62% pooled, matching iteration 3); spring (the season the user was most disappointed with) closed most of its gap to summer/autumn, with all four named worst-in-spring genera improving (Fraxinus +10.6pp, Ulmus +5.1pp, Juglans +19.7pp, Tilia +14.7pp). Promotion criterion applied explicitly (beats iteration 3 on the temperate genera specifically, not just the mean): satisfied without exception -- promoted genus_classifier_v4.tflite to the canonical path (train/genus_classifier.tflite, genus_classifier_keras/, training_report.json, export_report.json, eval/results/*), eval/GATE unchanged (GATE-MODEL: PASS). Iterations 1-3 preserved separately, untouched. Section 14.2's saturation finding means a fifth iteration repeating this recipe (more epochs, bigger corpus, same backbone) is not expected to repeat this size of gain -- the next lever, if the ADR calls for one, is more data for the genera still short of D-02, or the parked Tela Botanica/Wikimedia Commons sources (Section 13.3)."
affects: [01-05, 01-06]
...
duration: ~50min for iteration 1 (two training runs ~245s and ~576s CPU-only), plus ~5.5h for iteration 2 (corpus expansion ~2.5h, two training attempts ~88min + ~84min including a discarded run, evaluation and write-up), plus ~2 sessions / roughly 9-10h wall-clock for iteration 3 (corpus expansion to 194,653 images across two interrupted/hardened runs including bug-fix time, ~4h46min training, export/evaluation/write-up/promotion in this final session) -- across three sessions total, no restart of already-completed work in any of them -- plus iteration 4 (source reconnaissance ~1.5h, corpus expansion ~3.2h across a killed-and-rescoped run plus a Pinus-only pass, training 23.47h wall-clock (EfficientNetB0, 28 epochs), export/evaluation/comparison/promotion/write-up ~1h), spanning a subscription session-limit interruption mid-training with no work lost (raw numbers committed at each stage)
...
**Four iterations. Iteration 1: MobileNetV3-Small on 220 images/class found 0/34 genera clearing the D-02 95% top-3 bar (closest, Olea, missing by one test image). The user rejected that no-go as premature; iteration 2 (MobileNetV3-Large, ~2,000 images/class, season-stratified, ~200-image test split) still found 0/34 clearing the bar, but 32/34 genera improved by a mean of +19.9pp top-3 — a data-limited result, not an approach-limited one. Iteration 3 (MobileNetV3-Large, iteration 2's exact hyperparameters held constant, corpus expanded 3.05x to 194,653 images at each class's real per-class ceiling) found 1 of 34 genera (Tamarix, 95.04%) clearing the bar for the first time, with the other 33 genera improving by a mean of +6.74pp top-3 (2→3) — diminishing but not exhausted, confounded by a never-saturating epoch budget (`EarlyStopping` never triggered). Iteration 4 (further deviation: user asked for a rebalance targeted at the Île-de-France temperate genera he actually surveys, disappointed by their weakness and by spring specifically) fixed a real fetch-pipeline bug (an early-exit heuristic and a season-agnostic walk that had silently capped the corpus's autumn coverage for Acer/Pinus/Prunus at near-zero — not, as iteration 3 concluded, a real GBIF scarcity), raised the per-class target to 10,000 for the 20 weak temperate genera specifically, switched to EfficientNetB0, and raised the epoch budget 2.5x. Result: corpus grew to 265,546 images; Acer/Pinus/Prunus autumn coverage went from 66/30/60 to 2,500/2,500/1,500, resolving Section 12.6's "cannot tell" verdict (Pinus's autumn cell now clears D-02 in isolation, 95.35%); the Île-de-France-targeted genera improved by a mean of +4.93pp top-3 (21/21 positive) against +1.65pp for the untouched Mediterranean group — the targeted rebalance worked roughly 3x better on the genera it targeted. Training reached genuine saturation this time (`EarlyStopping` still didn't fire on `val_top3`, but `val_loss` shows overfitting from epoch 15 on) — no longer schedule-limited. Still 1/34 genera (Tamarix, 96.88%) clears D-02; no new partial-go. All four iterations' full numbers are preserved; iteration 4's model (8.24MB, up from 6.13MB) is promoted to the canonical path after genuinely beating iteration 3 on the temperate genera specifically, not just the mean. `GATE-MODEL: PASS`.**
