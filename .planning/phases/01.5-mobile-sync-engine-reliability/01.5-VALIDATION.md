...
| 01.5-09-T1 | 09 | 3 | REQ-AUD-photos | T-01.5-33 | Preview state image / loading / missing / unavailable decided by a pure helper; unavailable excluded from ensure candidates | unit | `cd mobile && npx jest --config jest.unit.config.js src/screens/survey-screen-helpers.test.ts` | ✅ (extended) | ⬜ pending |
| 01.5-09-T2 | 09 | 3 | REQ-AUD-photos | T-01.5-32, T-01.5-34 | expo-image rendering; remote/missing visible; 'Photo non disponible' static state (no spinner); dev-only simulate control | build + coverage | `npm --workspace mobile run test:unit:coverage && npx expo export` (plan 09 Task 2) + grep gates | ✅ screens | ⬜ pending |
