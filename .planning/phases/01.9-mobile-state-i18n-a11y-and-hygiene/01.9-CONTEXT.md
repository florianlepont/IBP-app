...
- **D-05, map.**
  ...
  - **Mobile, clustering:** use `supercluster@9.1.0` (owner approved). It is JavaScript-only, so there is no native rebuild. It is ESM-only, so Jest needs a `moduleNameMapper` to its dist bundle.
