...
export class IbpRulesService {
  validateDraft(
    ...
    const thresholdRegion = this.resolveThresholdRegion(
      this.normalizeRegion(regionVersion),
    ...
    if (!region) {
      issues.push(
        this.issue(
          ...
          "region_version is required and must be ACA or M",
          true,
        ),
      )
    }
  ...
  private normalizeFactors(
    ...
    for (const key of FACTOR_KEYS) {
      ...
      if (score === null) {
        context.issues.push(
          this.issue("factor_invalid_raw", `factor ${key} has invalid raw input`, true, key),
        )
        continue
      }
      ...
      if (!this.isAllowedScore(key, score)) {
        ...
        context.issues.push(
          this.issue(
            ...
            `factor ${key} must resolve to one of [${allowedValues}]`,
            true,
            key,
          ),
        )
  ...
  private scoreFactorB(raw: Record<string, unknown>): number | null {
    ...
    const coverPercent = this.pickNumber(raw, [
  ...
  private scoreFactorF(raw: Record<string, unknown>, context: NormalizeContext): number | null {
    ...
    for (const g of groups) {
      ...
      if (n > 2) {
        context.issues.push(
          this.issue(
            "factor_f_group_capped",
            "factor F group count capped to 2 trees/ha",
            false,
            "F",
          ),
        )
  ...
  private addNonBlockingConsistencyWarnings(
  ...
  private applyWarningsToCanonicalFactors(
    issues: IbpValidationIssue[],
    factorResults: Record<string, FactorCanonical>,
  ): void {
  ...
  private resolveThresholdRegion(
    region: RegionVersion | undefined,
