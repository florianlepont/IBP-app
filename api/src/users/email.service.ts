...
export class EmailService {
  ...
  async sendEmailChangeConfirmation(message: EmailChangeMessage): Promise<void> {
    ...
    const expiresAtText = Number.isNaN(expiresAtDate.getTime()) ? message.expiresAtIso : expiresAtDate.toUTCString();
    ...
    if (!this.smtpEnabled || !this.transporter) {
      this.logger.log(`[DEV EMAIL] to=${message.toEmail} subject="${subject}" token=${message.token}`);
      return;
    }
