...
export class UsersService {
  ...
  async patchMe(user: AuthenticatedUser, body: PatchMeBody): Promise<MeResponse> {
    ...
    const emailChangeExpiresAt = emailChangeRequested ? new Date(Date.now() + EMAIL_CHANGE_TTL_MS).toISOString() : null;
  ...
  async confirmEmailChange(user: AuthenticatedUser, token: string): Promise<MeResponse> {
    ...
    const expiresAtMs = current.email_change_expires_at ? Date.parse(current.email_change_expires_at) : NaN;
  ...
  async uploadProfilePicture(
    ...
    if (current.profile_picture_storage_key && current.profile_picture_storage_key !== storageKey) {
      await rm(this.storagePathForKey(current.profile_picture_storage_key), { force: true }).catch(() => undefined);
    }
