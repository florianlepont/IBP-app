...
describe('Epic E - Search and Reports (e2e)', () => {
  ...
  it('filters my surveys by query/date/status via GET /v1/surveys', async () => {
    ...
    await db.query(`UPDATE surveys SET updated_at = '2026-03-01T10:00:00Z'::timestamptz WHERE id = $1`, [surveyA]);
    await db.query(`UPDATE surveys SET updated_at = '2026-03-06T10:00:00Z'::timestamptz WHERE id = $1`, [surveyB]);
    await db.query(`UPDATE surveys SET updated_at = '2026-03-10T10:00:00Z'::timestamptz WHERE id = $1`, [surveyC]);
    ...
    const invalidDateIgnored = await request(app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${accessToken}`)
      .expect(200);
    const invalidDateIds = (invalidDateIgnored.body.items as Array<{ id: string }>).map((item) => item.id);
  ...
  it('supports reporting and moderator review workflow via /v1/reports', async () => {
    ...
    const ownerLogin = await request(app.getHttpServer())
      .post('/v1/auth/login')
      .send({ email: ownerEmail, password: 'demo123' })
      .expect(201);
    ...
    const reporterLogin = await request(app.getHttpServer())
      .post('/v1/auth/login')
      .send({ email: reporterEmail, password: 'demo123' })
      .expect(201);
    ...
    const moderatorLogin = await request(app.getHttpServer())
      .post('/v1/auth/login')
      .send({ email: moderatorEmail, password: 'demo123' })
      .expect(201);
    ...
    const openReports = await request(app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${moderatorToken}`)
      .expect(200);
    expect((openReports.body.items as Array<{ id: string }>).some((item) => item.id === reportId)).toBe(true);
    ...
    const reviewedReports = await request(app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${moderatorToken}`)
      .expect(200);
    const reviewedRow = (reviewedReports.body.items as Array<{ id: string; reviewed_by: string | null }>).find(
