...
import { Alert, Image, NativeScrollEvent, NativeSyntheticEvent, Pressable, ScrollView, StyleSheet, Text, TextInput, View, useWindowDimensions } from 'react-native';
...
import { FACTOR_TITLES, REGION_OPTIONS, VEGETATION_STAGE_OPTIONS_BY_REGION, defaultVegetationStageForRegion, normalizeVegetationStageForRegion } from '../app/constants';
...
import { formatSurveySyncDisplayLabel, formatSurveyWorkflowStatusLabel, resolveSurveySyncDisplay, resolveSurveyWorkflowStatus } from '../app/survey-logic';
...
export function SurveyDetailScreen({
  ...
  const heroPrimaryMetricLabel = displayedScores ? (useLocalDraftView ? 'Local draft score' : 'IBP total') : 'Factors ready';
  const heroPrimaryMetricValue = displayedScores ? formatPoints(displayedScores.ibp_total) : completedFactorCountForSubmit !== null ? `${completedFactorCountForSubmit}/10` : '--';
  ...
  return (
    ...
    >
      <View style={styles.detailHeroStickyWrap}>
        <View style={[styles.detailHeroCard, shouldShowCompressedHero ? styles.detailHeroCardCompressed : null]}>
            ...
            <View style={styles.detailHeader}>
              ...
              >
                ...
                <View style={styles.detailHeroStatusRow}>
                  ...
                  <View style={[styles.detailHeroStatusPill, styles.detailHeroStatusPillNeutral]}>
                    <Text style={styles.detailHeroStatusPillText}>{selectedSurvey.visibility === 'public' ? 'Public' : 'Private'}</Text>
                  </View>
                </View>
              </Pressable>
        ...
        {heroSubmitState !== 'ready' ? (
          <View style={[styles.detailHeroProgressCard, shouldShowCompressedHero ? styles.detailHeroProgressCardCompact : null]}>
            ...
            <View style={styles.detailHeroProgressTrack}>
              <View style={[styles.detailHeroProgressFill, { width: completionRate === 0 ? 0 : `${completionRate}%` }]} />
            </View>
            {!shouldShowCompressedHero ? (
              <View style={styles.detailHeroProgressFooter}>
                <View style={styles.heroMetaPill}>
                  <Ionicons name="time-outline" size={13} color="#D7E3C0" />
                  <Text style={styles.heroMetaText}>Updated {formatDateTime(selectedSurvey.updated_at)}</Text>
                </View>
                <View style={styles.heroMetaPill}>
                  <Ionicons name="images-outline" size={13} color="#D7E3C0" />
                  <Text style={styles.heroMetaText}>{selectedSurveyAttachments.length} photo(s)</Text>
                </View>
                {selectedSurvey.status !== 'submitted' ? (
                  <View style={styles.heroMetaPill}>
        ...
        {selectedSurvey.status !== 'submitted' ? (
          isSubmitReady && shouldShowCompressedHero ? (
            <Pressable style={styles.detailHeroSubmitButtonCompact} onPress={() => void onSubmitSurvey(selectedSurvey.id)}>
            ...
            >
              <View style={styles.detailHeroSubmitHeader}>
                <View style={styles.detailHeroSubmitHeaderCopy}>
                  ...
                  {!shouldShowCompressedHero ? <Text style={styles.detailHeroSubmitBody}>{heroSubmitBody}</Text> : null}
                </View>
                {isSubmitReady ? (
                  <Pressable style={styles.detailHeroSubmitButtonInline} onPress={() => void onSubmitSurvey(selectedSurvey.id)}>
                    <Ionicons name="paper-plane-outline" size={15} color={brandColors.forest} />
          ...
          {heroMode === 'photo' && hasPhotoSlides ? (
            ...
            <Pressable style={styles.detailHeroMain} onPress={handleOpenParcels} disabled={!canEditSurvey}>
              ...
              <View style={styles.detailHeroOverlayBadge}>
                ...
                <Text style={styles.detailHeroOverlayBadgeText}>{canEditSurvey ? 'Tap map to edit parcels' : 'Map preview'}</Text>
              </View>
            </Pressable>
          )}
      ...
      <View style={styles.filterChipsRow}>
        <SurfaceChip label="Summary" active={surveyDetailTab === 'summary'} onPress={() => setSurveyDetailTab('summary')} />
        <SurfaceChip label="Events" active={surveyDetailTab === 'events'} onPress={() => setSurveyDetailTab('events')} />
        <SurfaceChip label="Debug" active={surveyDetailTab === 'debug'} onPress={() => setSurveyDetailTab('debug')} />
      </View>
          ...
          <View style={styles.detailMetadataCard}>
            <View style={styles.sectionHeader}>
              <View style={styles.sectionHeaderCopy}>
                ...
                <Text style={styles.panelBody}>Region version and vegetation stage used by the scoring rules.</Text>
              </View>
              {canEditSurvey ? (
          ...
          <View style={styles.factorTilesCard}>
            <View style={styles.sectionHeader}>
              <View style={styles.sectionHeaderCopy}>
                ...
                <Text style={styles.panelBody}>Open factors to update observations and live scores.</Text>
              </View>
            </View>
            {showFactorLoadingHint ? <Text style={styles.rowMeta}>Loading factors...</Text> : null}
            {displayedScores ? (
              <>
                {useLocalDraftView ? <Text style={styles.rowMeta}>Showing local draft score from latest edits.</Text> : null}
                ...
                <View style={styles.factorTotalsRow}>
                  ...
                  <View style={styles.factorTotalPill}>
                    <Text style={styles.factorTotalText}>Context {formatPoints(displayedScores.ibp_contexte)}</Text>
                  </View>
                </View>
                  ...
                  {displayedFactorEntries.map(([factorCode, factor]) => {
                    ...
                    return (
                      ...
                      >
                          ...
                          <View style={styles.factorTileIdentity}>
                            ...
                            <View style={[styles.factorTileIconWrap, factorCompleted ? styles.factorTileIconWrapCompleted : styles.factorTileIconWrapPending]}>
                              <Ionicons
                                ...
                                size={16}
                                color={factorCompleted ? brandColors.forest : brandColors.textSecondary}
                              />
                            </View>
                          </View>
                        ...
                        <Text style={[styles.factorTileCode, factorCompleted ? styles.factorTileClassCompleted : styles.factorTileClassPending]}>
                          {factor.selected_class}
                        </Text>
                        {factor.warnings.length > 0 ? <Text style={styles.factorTileWarning}>Has warning</Text> : null}
                      </Pressable>
                    );
                  })}
                </View>
          ...
          <View style={styles.actionPanel}>
            <View style={styles.sectionHeader}>
              <View style={styles.sectionHeaderCopy}>
                ...
                <Text style={styles.panelBody}>Visibility, deletion and sync recovery controls.</Text>
              </View>
            </View>
            ...
            {selectedSurvey.sync_state === 'failed' ? (
              <View style={styles.actionButtonsRow}>
                <ActionButton label="Retry now" icon="refresh-outline" variant="primary" onPress={() => void onRetrySurvey(selectedSurvey.id)} />
                <ActionButton label="Discard local change" icon="close-circle-outline" variant="danger" onPress={() => void onDiscardSurvey(selectedSurvey.id)} />
              </View>
            ) : null}
          </View>
        </View>
        ...
        <View style={styles.detailSection}>
          <View style={styles.eventsCard}>
            <View style={styles.sectionHeader}>
              ...
              <ActionButton label="Reload" icon="refresh-outline" variant="neutral" onPress={() => void onLoadSurveyEvents(selectedSurvey.id)} />
            </View>
      ...
      {surveyDetailTab === 'debug' ? (
        <View style={styles.detailSection}>
          <View style={styles.debugCard}>
            ...
            {selectedSurvey.last_sync_error ? <Text style={styles.rowMeta}>last error: {selectedSurvey.last_sync_error}</Text> : <Text style={styles.rowMeta}>No sync error reported.</Text>}
            ...
            {selectedSurveyAttachments.length === 0 ? (
              ...
              selectedSurveyAttachments.map((attachment, index) => (
                <View key={`debug-attachment-${attachment.id}`} style={styles.debugAttachmentCard}>
                  ...
                  <Text style={styles.rowMeta}>size: {attachment.size_bytes} bytes ({Math.round(attachment.size_bytes / 1024)} KB)</Text>
