...
describe("initLocalDb", () => {
  test("runs CREATE TABLE statements", async () => {
    ...
    expect(mockDb.execAsync).toHaveBeenCalledWith(
      expect.stringContaining("CREATE TABLE IF NOT EXISTS local_surveys"),
    )
    expect(mockDb.execAsync).toHaveBeenCalledWith(
      expect.stringContaining("CREATE TABLE IF NOT EXISTS sync_queue"),
    )
    expect(mockDb.execAsync).toHaveBeenCalledWith(
      expect.stringContaining("CREATE TABLE IF NOT EXISTS local_attachments"),
    )
    expect(mockDb.execAsync).toHaveBeenCalledWith(
      expect.stringContaining("CREATE TABLE IF NOT EXISTS local_meta"),
    )
  })
...
describe("createLocalDraft", () => {
  ...
  test("inserts into local_surveys", async () => {
    ...
    const insertCall = mockDb.runAsync.mock.calls.find((c) =>
      (c[0] as string).includes("INSERT INTO local_surveys"),
    )
...
describe("updateLocalDraft", () => {
  ...
  test("inserts a new sync_queue row", async () => {
    ...
    const insertQueueCall = mockDb.runAsync.mock.calls.find((c) =>
  ...
  test("updates local_surveys row", async () => {
    ...
    const updateCall = mockDb.runAsync.mock.calls.find((c) =>
      (c[0] as string).includes("UPDATE local_surveys"),
    )
...
describe("queueDeleteSurvey", () => {
  ...
  test("deletes survey locally and queues a delete operation", async () => {
    ...
    const deleteSurveyCall = mockDb.runAsync.mock.calls.find((c) =>
      (c[0] as string).includes("DELETE FROM local_surveys"),
    )
  ...
  test("also deletes local attachments", async () => {
    ...
    const deleteAttCall = mockDb.runAsync.mock.calls.find((c) =>
      (c[0] as string).includes("DELETE FROM local_attachments"),
    )
...
describe("queueLocalAttachment", () => {
  ...
  test("inserts into local_attachments and sync_queue", async () => {
    ...
    const insertAttCall = mockDb.runAsync.mock.calls.find((c) =>
      (c[0] as string).includes("INSERT INTO local_attachments"),
    )
...
describe("retrySurveyNow", () => {
  ...
  test("resets status and next_retry_at for failed queue items", async () => {
    ...
    const updateCall = mockDb.runAsync.mock.calls.find((c) =>
      (c[0] as string).includes("UPDATE sync_queue"),
    )
    expect(updateCall).toBeDefined()
...
describe("discardSurveyLocalChanges", () => {
  test("deletes pending queue rows for survey", async () => {
    ...
    const deleteQueueCall = mockDb.runAsync.mock.calls.find((c) =>
      (c[0] as string).includes("DELETE FROM sync_queue"),
    )
...
describe("markSurveyExpiredLocally", () => {
  test("runs an UPDATE setting status to expired", async () => {
    ...
    const updateCall = mockDb.runAsync.mock.calls.find((c) =>
      (c[0] as string).toLowerCase().includes("update local_surveys"),
    )
    expect(updateCall).toBeDefined()
